...
# this implements checking only (no minting) but it would be not hard to add
# minting also if someone gets bored
#
...
function hc_check() {
    tbits=`hc_bits $1`
    if ! hc_ok `echo $1 | cut -d: -f 1-3 --output-delimiter ' '` "$2" "$3"; 
    then return 1; 
    fi
...
function hc_ok() {
    if [ $1 != "0" ]; then echo unsupported version 1>&2; return 1; fi
    minted=`hc_time "$2" -u`
...
now=`date +%s`
adjust=0
while getopts "b:cde:fg::r:t:uv" opt; do
    case $opt in
    b) case $OPTARG in
