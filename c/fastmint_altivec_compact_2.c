...
#if !defined( COMPACT ) && defined(__POWERPC__) && defined(__ALTIVEC__) && defined(__GNUC__)
...
unsigned long minter_altivec_compact_2( int bits, int* best, 
					unsigned char *block, 
					const uInt32 IV[5], int tailIndex, 
					unsigned long maxIter, 
					MINTER_CALLBACK_ARGS ) {
	...
	*/
	...
	if ( vec_any_ne( vec_and( 
			     vec_cmpeq(vec_and(A, vBitMaskLow),
				       (vector unsigned int) (0)), 
			     vec_cmpeq(vec_and(A, vBitMaskHigh), 
				       (vector unsigned int) (0)) ), 
			 (vector unsigned int) (0)) ) {
	    uInt32 IA, IB;
