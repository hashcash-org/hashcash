...
should output: I<a9993e364706816aba3e25717850c26c9cd0d89d>.

=item C<echo -n abc | sha1 ->

Equivalent to above.  The filename of I<-> means read from standard input.

=item C<echo -n abc | sha1 test.txt ->

You can also mix filenames and reading from standard input with the I<->
pseudo file.  The above command first hashes file C<test.txt> then reads the
string abc from standard in.

=back

=head1 Verifying Hashcash
...
From: Someone E<lt>test@test.invalidE<gt>
To: Adam Back E<lt>adam@cypherspace.orgE<gt>
...
=item C<echo -n 0:030626:adam@cypherspace.org:6470e06d773e05a8 | sha1>

=item C<00000000c70db7389f241b8f441fcf068aead3f0>
...
C<sha1> returns success (exit code 0) normally.
...
md5sum(1), hashcash(1), http://www.hashcash.org/,
