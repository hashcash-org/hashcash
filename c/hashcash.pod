...
=head2 Mint (create) tokens:

B<hashcash> I<-m> [ [ I<options> ] [ I<resource> ... ] ... ]

=head2 Check tokens:

B<hashcash> I<-c> [ [ I<options> ] I<-r resource> ... ] [ I<token> ... ] 
...
The I<-m> flag must be given to mint a token.
...
option.  If no collision size is specified, the default is 20 bits.  I<-b
default> can be used to get the default size.
...
is returned on exit.  The I<-n> flag can be used to request that the
resource name in the token is parsed out and displayed; exit code
unchecked (exit code 2) is returned on exit.  The I<-l> flag can be
...
The program will only return exit codes valid or invalid if the I<-c> flag
is used, and both of the options, I<-d>, I<-r resource> are used.  These are
...
(exit code 2) on exit.  (See also the I<-y> flag.)
...
unexpired tokens also, and with the I<-j resource> flag to purge only
...
also specify the default with I<-b default> which is useful for use
with I<-s>.

=item I<-r resource>

When minting tokens, the resource name (recipient's email address) to
mint the token against can be given either with I<-r resource> or as
...
Expiry period for spent tokens.  While checking tokens (using the I<-c>
...
While minting tokens, the I<-e> flag can have an effect on the
...
the description of the I<-e> flag.  Valid widths are 2,4,6,8,10 or 12
...
database file is F<database.db> in the current directory.  Only
otherwise valid tokens will be stored in the database.  Only fully
validated tokens will be stored in the database, unless the I<-y>
option is given.
...
Note the I<-E>, I<-W> and I<-S> type of match flags also apply to
...
Similar effect to I<-X> for minting and checking except you get to
choose your own header string.  For example I<-x 'X-Hashcash: '> has
the same effect as I<-X>.

=item I<-i>

When checking and using the I<-X> or I<-x> flag, ignore the blank line
...
When checking tokens, allow wildcards I<*> in the resource name to
...
is the default.  See also I<-S>, I<-E> and I<-C>

=item I<-S>

When checking tokens use simple text compare to compare resource names
to those in tokens.  See also I<-W>, <-E> and I<-C>.
...
string.  (Note even if BSD regular expressions are used POSIX style
...
the I<-e> option is 28 days (ie tokens expire after 4 weeks).

First mint and then add a token:

=over 4

=item C<hashcash -m -b 10 foo -e 1m E<gt> token>
...
=item C<hashcash -m -b 10 foo E<gt> token>
