...
=head2 Count partial preimage bits:
...
If a required preimage size is given with the I<-b> flag, the stamps value
...
preimage are counted and displayed. The I<-n> flag can be used to
...
The I<-s> flag requests measurement of how many preimage can be
...
requested preimage size would take to compute, if I<-s> given by
...
=item I<bits> = how many bits of partial-preimage the stamp is claimed to have
